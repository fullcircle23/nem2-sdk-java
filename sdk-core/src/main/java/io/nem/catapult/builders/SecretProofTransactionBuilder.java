...
public final class SecretProofTransactionBuilder extends TransactionBuilder {
     ...
     */
    protected SecretProofTransactionBuilder(final SignatureDto signature, final KeyDto signerPublicKey, final short version, final EntityTypeDto type, final AmountDto fee, final TimestampDto deadline, final LockHashAlgorithmDto hashAlgorithm, final Hash256Dto secret, final UnresolvedAddressDto recipientAddress, final ByteBuffer proof) {
        super(signature, signerPublicKey, version, type, fee, deadline);
        this.secretProofTransactionBody = SecretProofTransactionBodyBuilder.create(hashAlgorithm, secret, recipientAddress, proof);
    }
     ...
     */
    public static SecretProofTransactionBuilder create(final SignatureDto signature, final KeyDto signerPublicKey, final short version, final EntityTypeDto type, final AmountDto fee, final TimestampDto deadline, final LockHashAlgorithmDto hashAlgorithm, final Hash256Dto secret, final UnresolvedAddressDto recipientAddress, final ByteBuffer proof) {
        return new SecretProofTransactionBuilder(signature, signerPublicKey, version, type, fee, deadline, hashAlgorithm, secret, recipientAddress, proof);
    }
